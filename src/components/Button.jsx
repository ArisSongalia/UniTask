...
function Button({ text = 'Button', onClick, className = '', type = 'button', iconClassName = ''}) {
  return (
    <button
      ...
      hover:bg-green-700 hover:text-white border-opacity-60 border border-green-800 text-green-800 p-2 ${className}`}
      onClick={onClick}
    >
...
function ButtonIcon({ text = 'Button', onClick, className = '', type = 'button', dataFeather = '', iconClassName = ''}) {
  return (
    <button
      type={`${type}`}
      className={`group flex text-green-800 bg-green-50  px-1 py-1 items-center rounded-full pr-4 text-xs font-bold h-fit
