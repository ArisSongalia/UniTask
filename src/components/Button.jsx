...
function Button({ text = 'Button', onClick, className = '', type = 'button'}) {
  return (
    <button
      type={`${type}`}
      className={`justify-center bg-green-700 text-xs px-4 font-bold rounded-md py-3 w-full
      hover:bg-green-800 hover:text-white border-opacity-60 border border-green-600 text-white p-2 ${className}`}
      onClick={onClick}
    >
...
function ButtonIcon({ text = 'Button', onClick, className = '', type = 'button', dataFeather = '', iconClassName = ''}) {
  return (
    <button
      ...
      hover:bg-green-800 hover:text-white text-green-800  ${className}`}
      onClick={onClick}
    >
