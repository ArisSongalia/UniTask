...
function ButtonIcon({ text = 'Button', onClick, className = '', type = 'button', dataFeather = '', iconClassName = ''}) {
  return (
    <button
      type={`${type}`}
      className={`group flex text-green-800 bg-green-50 px-2 items-center rounded-full pr-4 text-xs font-bold h-fit
      hover:bg-green-700 hover:text-white border-opacity-60 border justify-center ${className}`}
      onClick={onClick}
    >
      <Icon dataFeather={dataFeather} className={`group-hover:text-white text-white ${iconClassName}`} />
      <p className='font-bold text-xs pr-4'>{text}</p>
    </button>
  );
}
